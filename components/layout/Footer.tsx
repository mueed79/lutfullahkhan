...
export default function Footer() {
  ...
  /* ── FOOTER ──────────────────────────────────────────────────
     Height:            h-[800px] (mobile) / md:h-[1272px] (desktop) ← adjust section height
     Audio file:        /audio/mehdi-hassan.mp3 ← swap to change the footer track
     Painting opacity:  opacity-65 on the Image ← lower = darker painting
     Stars opacity:     opacity-40 on the video ← lower = less stars effect
     Dark overlay:      bg-black/20 ← raise to darken the whole footer
     Legal bar height:  md:h-[140px] ← bottom bar height                        */
  return (
    <footer className="relative h-[800px] md:h-[1272px] bg-[#1c1917] overflow-hidden w-full">
      <audio ref={audioRef} src="/audio/mehdi-hassan.mp3" loop /> {/* ← swap audio file here */}

      {/* 1. Background layers: painting image + stars video + dark overlay */}
      <div className="absolute inset-0">
        <Image
          src="/footer.png"
          alt="Salman Toor Painting"
          fill
          className="object-cover opacity-65" /* ← opacity-65: painting visibility */
        />
        <video
          autoPlay
          loop
          muted
          playsInline
          className="absolute inset-0 w-full h-full object-cover mix-blend-screen opacity-40 pointer-events-none" /* ← opacity-40: stars intensity */
        >
          <source src="/stars.mp4" type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-black/20" /> {/* ← bg-black/20: overall darkness */}
      </div>

      {/* 2. Main narrative text — "btw, you're still in the good old days."
          Main text size: text-[34px] | Sub text size: text-[28px]
          Sub text offset (desktop): md:left-[300px] md:top-[160px] ← reposition */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none px-6">
        <div className="relative w-full max-w-[800px] h-[400px] md:h-[300px] flex flex-col items-center md:items-start text-center md:text-left">
          <div className="relative md:absolute left-0 top-0 mb-8 md:mb-0">
            <h2 className="text-[28px] md:text-[34px] tracking-tight leading-tight text-white"> {/* ← text-[34px]: main headline size */}
      ...
      {/* 3. Audio track info + play/mute button
          Desktop position: md:left-[80px] md:bottom-[320px] ← reposition
          Song title size:  text-[36px] | Artist size: text-[20px]             */}
      <div className="absolute left-1/2 -translate-x-1/2 md:translate-x-0 md:left-[80px] bottom-[230px] md:bottom-[320px] flex items-center gap-4 w-full px-6 justify-center md:justify-start">
        <div className="flex flex-col items-center md:items-start">
          <div className="flex items-center gap-3">
            <span className="font-heading text-[28px] md:text-[36px] text-white leading-none whitespace-nowrap"> {/* ← text-[36px]: song title size */}
          ...
          <span className="text-[16px] md:text-[20px] text-white font-regular mt-2 opacity-60"> {/* ← text-[20px]: artist name size */}
            Mehdi Hassan
          </span>
        </div>
      </div>

      {/* 4. Painting credit — bottom right */}
      ...
      {/* 5. Legal bar — copyright + logo
          Bar height: md:h-[140px] | Side padding: md:px-[80px]
          Copyright text size: text-[16px]                                      */}
      <div className="absolute bottom-0 left-0 w-full h-auto md:h-[140px] flex flex-col md:flex-row items-center justify-between px-6 md:px-[80px] py-10 md:py-0 border-t border-white/5 bg-black/20 backdrop-blur-md gap-6">
        ...
        <div className="scale-[0.7] md:scale-[0.8] origin-center md:origin-right"> {/* ← scale-[0.8]: logo size in footer */}
          <Logo light={true} />
        </div>
      </div>
    </footer>
